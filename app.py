...
# ------------------------------------------------------------
# Rutas y carga de modelo (ahora app.py está en la raíz)
# ------------------------------------------------------------
ROOT = Path(__file__).resolve().parent  # raíz del repo dentro del contenedor: /app
DEFAULT_MODEL = (ROOT / "models" / "Modelo_yolov8_pklot2.pt").resolve()
...
def load_model(model_path: str) -> YOLO:
    ...
    if not Path(model_path).exists():
        raise FileNotFoundError(
            ...
            f"Sug.: exporta MODEL_PATH o coloca el .pt en {DEFAULT_MODEL}"
        )
...
with st.sidebar:
    ...
    conf = st.slider("Nivel de confianza", 0.1, 0.9, 0.5, 0.05)
    iou = st.slider("Nivel de superposición (IoU)", 0.1, 0.9, 0.7, 0.05)
    imgsz = 640
...
if mode == "Imagen":
    ...
    if up is not None:
        img_arr = np.frombuffer(up.read(), np.uint8)
    ...
    upv = st.file_uploader("Arrastra o selecciona un video (MP4/MOV/AVI/MKV)", type=["mp4", "mov", "avi", "mkv"])
    if upv is not None:
